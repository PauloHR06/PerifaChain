...
COMMENT ON TABLE users IS 'Cadastro de todos os perfis da plataforma.';
...
COMMENT ON TABLE artists IS 'Perfil estendido para quem é artista.';
...
COMMENT ON TABLE projects IS 'Projetos de captação (álbum, videoclipe, etc.).';
...
COMMENT ON TABLE loans IS 'Empréstimos feitos pelos investidores para projetos.';
...
COMMENT ON TABLE transactions IS 'Histórico de movimentações (on/off-chain).';
...
COMMENT ON TABLE revenue_sources IS 'Fontes de receita para royalties (streaming, shows, etc.).';
...
COMMENT ON TABLE docs IS 'Documentos de KYC e comprovações de projetos.';
